...
const mapWaitingRoomEntry = (entry: any) => ({
...
const mapActivePatient = (details: any) => {
...
const mapWsMessage = (envelope: any) => {
...
const ProviderTelemedicineChat = () => {
  ...
  const [activeConsultationId, setActiveConsultationId] = useState<any>(null);
  ...
  const activeConsultationIdRef = useRef<any>(null);
  // Stable ref to handleWsEvent so ws.onmessage never closes over a stale version
  const wsEventHandlerRef = useRef<any>(null);
  ...
  const handleAcceptPatient = useCallback(
    async (patient) => {
      ...
      setPatients((prev: any) => prev.filter((p: any) => p.id !== patient.id));
        ...
        case "message": {
          ...
          setWsMessages((prev: any) => {
            ...
            if (
              ...
              prev.some((m: any) => m.id === envelope.payload.message_id)
            )
              return prev;
            if (prev.some((m: any) => m.id === mapped.id)) return prev;
  ...
  const allMessages = React.useMemo(() => {
    ...
    const seen = new Set(dbMapped.map((m: any) => m.id));
    const newWs = wsMessages.filter((m: any) => !seen.has(m.id));
  ...
  const handlePrescriptionConfirm = (form: any) => {
  ...
  const handleReferralConfirm = (form: any) => {
  ...
  const handleLabConfirm = (form: any) => {
...
const ProviderTelemedicineChatWithBoundary = (props: any) => (
  <ErrorBoundaryWrapper
    fallback={(fallbackProps: any) => (
