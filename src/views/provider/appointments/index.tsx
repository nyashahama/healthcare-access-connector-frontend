...
const Appointments = () => {
  ...
  const appointmentStats = {
    ...
    noShows:
      appointments?.filter((apt: any) => apt.status === "no-show")?.length || 0,
  };
  ...
  const handleNewAppointmentChange = (field, value) => {
    setNewAppointment((prev: any) => ({ ...prev, [field]: value }));
  };

  const handleRescheduleChange = (field, value) => {
    setRescheduleForm((prev: any) => ({ ...prev, [field]: value }));
  };

  const handleCancelChange = (field, value) => {
    setCancelForm((prev: any) => ({ ...prev, [field]: value }));
  };
...
const AppointmentsWithBoundary = (props: any) => (
