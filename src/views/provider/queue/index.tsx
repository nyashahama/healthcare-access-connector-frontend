...
const Queue = () => {
  ...
  const handleFilterClick = (index: any) => {
    setQueueFilters((prev: any) =>
      prev.map((filter: any, i: number) => ({
        ...filter,
        active: i === index,
      }))
    );
  ...
  const handleClearQueue = () => {
    setModals((prev: any) => ({ ...prev, clearQueue: false }));
    showToast("Queue cleared successfully!", "success");
  };

  const handleCallNext = () => {
    setModals((prev: any) => ({ ...prev, callNext: false }));
    showToast("Next patient called!", "success");
  };

  const handleMarkAllSeen = () => {
    setModals((prev: any) => ({ ...prev, markAllSeen: false }));
    ...
    <div className="h-full">
      ...
      <div className="mb-6 flex items-center justify-between">
        ...
        <div className="flex space-x-3">
          ...
          <button
            onClick={() => setModals((prev: any) => ({ ...prev, addPatient: true }))}
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <QueueControls
          ...
          onCallNext={() => setModals((prev: any) => ({ ...prev, callNext: true }))}
          onMarkAllSeen={() =>
            setModals((prev: any) => ({ ...prev, markAllSeen: true }))
          }
          onClearQueue={() =>
            setModals((prev: any) => ({ ...prev, clearQueue: true }))
          }
        />
      </div>
      ...
      <AddPatientModal
        ...
        onClose={() => setModals((prev: any) => ({ ...prev, addPatient: false }))}
        onConfirm={handleAddPatient}
      />

      <ClearQueueModal
        isOpen={modals.clearQueue}
        onClose={() => setModals((prev: any) => ({ ...prev, clearQueue: false }))}
        onConfirm={handleClearQueue}
      />

      <CallNextModal
        isOpen={modals.callNext}
        onClose={() => setModals((prev: any) => ({ ...prev, callNext: false }))}
        onConfirm={handleCallNext}
      />

      <MarkAllSeenModal
        isOpen={modals.markAllSeen}
        onClose={() => setModals((prev: any) => ({ ...prev, markAllSeen: false }))}
