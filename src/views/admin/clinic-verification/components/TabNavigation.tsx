const TabNavigation = ({ selectedTab, setSelectedTab, clinics }: any) => {
  const tabs = [
    { id: "all", label: "All Clinics", count: clinics?.length || 0 },
    {
      ...
      label: "Pending",
      count:
        clinics?.filter((c: any) => c.verification_status === "pending").length || 0,
    },
    {
      id: "verified",
      label: "Verified",
      count:
        clinics?.filter((c: any) => c.verification_status === "verified").length ||
        0,
    },
    {
      id: "rejected",
      label: "Rejected",
      count:
        clinics?.filter((c: any) => c.verification_status === "rejected").length ||
        0,
    },
  ];
