...
const RegistrationQueue = ({ onApprove, onReject, onViewAll }: any) => {
  ...
  const [processingClinicId, setProcessingClinicId] = useState<any>(null);
  const [showConfirmationModal, setShowConfirmationModal] = useState(false);
  const [modalAction, setModalAction] = useState<any>(null); // 'approve' or 'reject'
  ...
  const handleApproveClick = (clinic: any) => {
  ...
  const handleRejectClick = (clinic: any) => {
    if (!currentUser || !currentUser.id) {
  ...
  const processApproval = async () => {
    ...
    if (result.success) {
      ...
      setPendingClinics((prev: any) =>
        prev.filter((c: any) => c.id !== selectedClinic.id)
      );
  ...
  const getDocumentCompletion = (clinic: any) => {
    ...
    const completedFields = requiredFields.filter((field: any) => field).length;
        ...
        <>
          <div className="overflow-x-auto">
            <table className="w-full min-w-[700px]">
              ...
              <tbody>
                {pendingClinics.slice(0, 4).map((clinic: any) => {
