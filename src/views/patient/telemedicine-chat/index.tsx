...
const TelemedicineChat = () => {
  ...
  const [wsMessages, setWsMessages] = useState<any[]>([]); // real-time messages from WS
  ...
  const consultationId = (activeConsultation?.id || currentConsultation?.id) as string | undefined;
      ...
      switch (envelope.type) {
        case "message": {
          ...
          setWsMessages((prev: any) => {
            ...
            if (
              payload.message_id &&
              prev.some((m: any) => m.id === payload.message_id)
            )
              return prev;
  ...
  const allMessages = React.useMemo(() => {
    ...
    // Merge, deduplicate
    const seen = new Set(dbMessages.map((m: any) => m.id));
  ...
  const handleSendMessage = async () => {
    ...
    if (wsRef.current?.isOpen?.()) {
      ...
      const result = await sendMessage(consultationId as string, {
        sender_role: "patient",
        message_type: "text",
        content: text,
      });
  ...
  const handleConnectToProvider = async (provider) => {
    ...
    if (result.success) {
      ...
      showToast(result.error || "Failed to request consultation", "error");
    }
  };
  ...
  const confirmEndChat = async () => {
    if (!consultationId) return;
    const result = await completeConsultation(consultationId as string);
    if (result.success) {
      disconnectWebSocket();
      setIsConnected(false);
      clearMessages();
  ...
  const handleBookFollowUp = () => {
    ...
    setTimeout(() => {
      navigate({ to: "/patient/find-clinic" });
    }, 1000);
  };
...
const TelemedicineChatWithBoundary = (props: any) => (
