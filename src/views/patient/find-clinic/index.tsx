...
const FindClinic = () => {
  ...
  const formatClinicData = (clinic: any) => {
  ...
  const filteredClinics = formattedClinics.filter((clinic: any) => {
    // Search filter
    if (
      searchQuery &&
  ...
  const handleShareClinic = (clinic: any) => {
    setSelectedClinic(clinic);
    setShareModalOpen(true);
  };

  const confirmDirections = (transport: any) => {
  ...
  const FilterButton = ({ icon, label, filterKey }: any) => (
    <button
      onClick={() =>
                  ...
                  <div className="space-y-3">
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1">
                        {clinic.languages?.map((lang: any, idx: number) => (
                          <span
                            key={idx}
