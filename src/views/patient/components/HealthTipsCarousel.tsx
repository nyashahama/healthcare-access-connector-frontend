...
const HealthTipsCarousel = () => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setCurrentSlide((prev: any) => (prev + 1) % tips.length);
    }, 5000);
  ...
  const prevSlide = () => {
    setCurrentSlide((prev: any) => (prev - 1 + tips.length) % tips.length);
  };
  ...
  const handleActionClick = (tip: any) => {
    switch (tip.action) {
      case "View Schedule":
      ...
      case "Check Milestones":
        ...
        setTimeout(() => {
                    navigate({ to: "/patient/health-records" });
        }, 1000);
        break;
      case "Set Reminder":
  ...
  const handleBookmarkTip = (tip: any) => {
