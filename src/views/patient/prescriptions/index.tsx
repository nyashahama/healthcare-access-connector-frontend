...
const Prescriptions = () => {
  ...
  const handleRequestRefill = (prescription: any) => {
    setSelectedPrescription(prescription);
    setModalState((prev: any) => ({ ...prev, refill: true }));
  };

  const handleShare = (prescription: any) => {
    setSelectedPrescription(prescription);
    setModalState((prev: any) => ({ ...prev, share: true }));
  };

  const handleDownload = (prescription: any) => {
    setSelectedPrescription(prescription);
    setModalState((prev: any) => ({ ...prev, download: true }));
  };

  const handleNewRequest = () => {
    setModalState((prev: any) => ({ ...prev, newRequest: true }));
  };

  const copyRxNumber = (rxNumber: any) => {
    navigator.clipboard.writeText(rxNumber);
    showToast("RX Number copied to clipboard", "info");
  };

  // Modal confirmation handlers
  const confirmRefill = (method: any) => {
    setModalState((prev: any) => ({ ...prev, refill: false }));
    showToast(
  ...
  const confirmShare = (method: any) => {
    setModalState((prev: any) => ({ ...prev, share: false }));
    showToast(`Prescription shared via ${method}`, "success");
  };

  const confirmDownload = (format: any) => {
    setModalState((prev: any) => ({ ...prev, download: false }));
    showToast(`Prescription downloaded as ${format.toUpperCase()}`, "success");
  };

  const confirmPrint = () => {
    setModalState((prev: any) => ({ ...prev, print: false }));
  ...
  const confirmNewRequest = () => {
    setModalState((prev: any) => ({ ...prev, newRequest: false }));
  ...
  return (
    <div className="rounded-[20px] bg-white p-6 shadow-sm dark:bg-navy-800">
      ...
      <RefillModal
        ...
        onClose={() => setModalState((prev: any) => ({ ...prev, refill: false }))}
        prescription={selectedPrescription}
      ...
      <ShareModal
        ...
        onClose={() => setModalState((prev: any) => ({ ...prev, share: false }))}
        prescription={selectedPrescription}
      ...
      <DownloadModal
        ...
        onClose={() => setModalState((prev: any) => ({ ...prev, download: false }))}
        prescription={selectedPrescription}
      ...
      <PrintModal
        ...
        onClose={() => setModalState((prev: any) => ({ ...prev, print: false }))}
        prescription={selectedPrescription}
