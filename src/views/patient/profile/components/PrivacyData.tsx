...
const PrivacyData = () => {
  ...
  useEffect(() => {
    ...
    patientService.getConsent(user.id).then((data) => {
      if (data) {
        setConsents((prev: any) => ({ ...prev, ...data }));
      }
    }).catch(() => {});
  }, [user?.id]);

  const handleConsentToggle = (consent: any) => {
  ...
  const handleFormChange = (field, value) => {
    setExportForm((prev: any) => ({ ...prev, [field]: value }));
  };

  const handleIncludeToggle = (item: any) => {
    setExportForm((prev: any) => ({
      ...prev,
      include: prev.include.includes(item)
        ? prev.include.filter((i: any) => i !== item)
        : [...prev.include, item],
    }));
  };
