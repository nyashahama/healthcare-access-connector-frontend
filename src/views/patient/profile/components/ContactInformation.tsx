...
const ContactInformation = () => {
  ...
  const patientData = usePatient() as any;
  const patient = patientData.patient as any;
  const patientLoading = patientData.loading;
  const updatePatientProfile = patientData.updatePatientProfile;
  const getCurrentPatientProfile = patientData.getCurrentPatientProfile;
  ...
  const parseDateForInput = (dateString: any) => {
    if (!dateString) return "";
    try {
      const date = new Date(dateString);
  ...
  const getOriginalValue = (field: any) => {
