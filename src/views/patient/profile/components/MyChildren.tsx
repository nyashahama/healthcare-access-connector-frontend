...
const MyChildren = () => {
  ...
  const handleDeleteClick = (child: any) => {
    setSelectedChild(child);
    setDeleteModalOpen(true);
  };

  const handleBookClick = (child: any) => {
    setSelectedChild(child);
    setAppointmentForm({
      type: "Regular Check-up",
      date: "",
  ...
  const handleFormChange = (field, value) => {
    setChildForm((prev: any) => ({ ...prev, [field]: value }));
  };

  const handleAppointmentFormChange = (field, value) => {
    setAppointmentForm((prev: any) => ({ ...prev, [field]: value }));
  };
  ...
  const confirmSave = () => {
    if (actionType === "add") {
      ...
      setChildren((prev: any) => [...prev, newChild]);
  ...
  const confirmDelete = () => {
    setChildren((prev: any) =>
      prev.filter((child: any) => child.id !== selectedChild.id)
    );
