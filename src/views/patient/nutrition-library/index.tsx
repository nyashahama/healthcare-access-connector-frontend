...
const NutritionLibrary = () => {
  ...
  // Handlers
  const toggleBookmark = (articleId: any) => {
    setBookmarkedArticles((prev: any) =>
      prev.includes(articleId)
        ? prev.filter((id: any) => id !== articleId)
        : [...prev, articleId]
    );
    showToast(
  ...
  const handleSaveOffline = (article: any) => {
    setSelectedArticle(article);
  ...
  const confirmShareArticle = (method: any) => {
...
Key Tips:
${article.tips.map((tip: any, i: number) => `${i + 1}. ${tip}`).join("\n")}
